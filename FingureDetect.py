...
options = vision.HandLandmarkerOptions(base_options=base_option, num_hands=1)                   # 모델 경로와 최대 손 개수 지정
...
while True:
    ...
    contour_lst, _ = cv2.findContours(mask,                             # Contour detection    
                                      cv2.RETR_EXTERNAL,                # RETR_EXTERNAL:바깥테두리만
    ...
    for hand in result.hand_landmarks:
        print("finger_angle_points:",finger_angle_points)
    ...
    for hand in result.hand_landmarks:
        ...
        for c in connections:
            #print("points[c.start]", points[c.start],center )
            cv2.line(frame, points[c.start], points[c.end], (0, 255, 0), 2)
            if center != (0,0):
                #cv2.line(frame, center, points[c.end], (255, 0, 0), 2)                   # 중심점
                print(math.dist(center, points[c.end]))
                if math.dist(center, points[c.end]) < 200 and total_finger_count < 4 and grab_finger_count > 3:
                    cv2.putText(frame, "Grab", (20, 105), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
                    cv2.putText(frame, f"Checked: {grab_finger_count}", (20, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
