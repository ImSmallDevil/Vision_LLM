...
base_option_f   = python.BaseOptions(model_asset_path="src/models/MediaPipe/face_landmarker.task")
options_f       = vision.FaceLandmarkerOptions(base_options=base_option_f, 
                                               num_faces=1)
face_detector   = vision.FaceLandmarker.create_from_options(options_f)                                
...
while True:
    ...
    result = hand_detector.detect(mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb))     # 프레임 내 손 탐지
    result_f = face_detector.detect(mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb))     # 프레임 내 손 탐지

    # Display Face Deteting dots
    if result_f.face_landmarks:
        #print("face_detector:", result_f.face_landmarks)
        h, w = frame.shape[:2]
        #print("face_detector:", h, w)
        for face in result_f.face_landmarks:
